// Test core functionality for Wonderful Sunset app
// This file tests the main components without UserNotifications
...
class CalendarViewModel {
    ...
    init() {
        // Skip notification permission check for testing
    }
...
func testWonderfulSunsetApp() {
    ...
    weatherService.getHourlyForecast(for: testLocation) { result in
        switch result {
        case .success(let forecast):
            print("✅ WeatherDataService test: Fetched \(forecast.count) forecast items")
        case .failure(let error):
            print("❌ WeatherDataService test: Failed - \(error.localizedDescription)")
        }
    }
    
    print("🎉 All core components test successfully!")
}
